...
class AppContext:
    ...
    # === AI 智能筛选(委托给 trendradar.ai.filter_runner) ===

    def run_ai_filter(self, interests_file: Optional[str] = None):
        """执行 AI 智能筛选完整流程

        实现在 trendradar/ai/filter_runner.py, 这里只是薄 wrapper 保持
        AppContext 接口稳定. 想直接调可以 import 那边的 run_ai_filter.
        """
        from trendradar.ai.filter_runner import run_ai_filter
        return run_ai_filter(self, interests_file=interests_file)
    ...
    def convert_ai_filter_to_report_data(
        ...
        实现在 trendradar/ai/filter_runner.py.
        """
        from trendradar.ai.filter_runner import convert_ai_filter_to_report_data
        return convert_ai_filter_to_report_data(
            self, ai_filter_result, mode=mode,
